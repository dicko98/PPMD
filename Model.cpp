...
void _STDCALL EncodeFile(ofstream & EncodedFile, ifstream & DecodedFile,int MaxOrder)
{
    ariInitEncoder();                       StartModelRare(MaxOrder);
    for (int ns=MinContext->NumStats; ;ns=MinContext->NumStats) {
        int c = DecodedFile.get();
        if (ns != 1) {
...
void _STDCALL DecodeFile(ofstream & DecodedFile, ifstream & EncodedFile,int MaxOrder)
{
